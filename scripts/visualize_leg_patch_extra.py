...
# Exclude images already used in previous run (by filename)
used = set()
for f in OUT_DIR.iterdir():
    if f.name.startswith("leg_rect_vis_") and f.suffix.lower() in {'.jpg', '.jpeg', '.png', '.bmp', '.avif'}:
        parts = f.stem.split('_')
        if len(parts) > 3:
            used.add('_'.join(parts[3:]))
        elif len(parts) > 2:
            used.add(parts[2])

remaining = [f for f in image_files if f.stem not in used]

# Select 2 random images not previously used
random.seed(43)
...
for idx, img_path in enumerate(selected):
    ...
    if len(kpts) > 0:
        ...
        out_path = OUT_DIR / f"leg_rect_vis_extra_{idx}_{img_path.stem}.jpg"
